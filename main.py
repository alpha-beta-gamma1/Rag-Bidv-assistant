...
def main():
    ...
    if args.command == 'ingest':
        ...
        for result in results:
            if 'error' in result:
                print(f"❌ {result.get('file_path')}: {result['error']}")
            else:
                print(f"✅ {result.get('file_path')}: {result.get('chunks_created')} chunks")

    elif args.command == 'query':
        logger.info(f"Processing query: {args.question}")
        result = rag_system.query(args.question)
        print(f"\n📝 Câu hỏi: {result.get('query', args.question)}")
        print(f"🤖 Trả lời: {result.get('response')}")
        # Uncomment nếu muốn show nguồn:
